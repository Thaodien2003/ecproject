...
public class AdminProductController {
    ...
    public String addProd(RedirectAttributes model,@ModelAttribute("entity") Product prod,@RequestParam("image_file") MultipartFile file) throws IllegalStateException, IOException {
        User user= (User) session.getAttribute("user");
        if(user ==null || !user.getAdmin()){
            return "redirect:/admin/login" ;
        }
        ...
        model.addFlashAttribute("message", "Add new product successful");
    ...
    public String updateProd(RedirectAttributes model,@ModelAttribute("entity") Product prod,@RequestParam("image_file") MultipartFile file, @RequestParam("uPrice") String uPrice) throws IllegalStateException, IOException, ParseException {
        User user= (User) session.getAttribute("user");
        if(user ==null || !user.getAdmin()){
            System.out.println("nulluser");
            return "redirect:/admin/login" ;
        ...
        model.addFlashAttribute("message","Update product successful");
        return "redirect:/admin/home/prod-manage";
    }
    // delete product

    @RequestMapping(value="/admin/prod-delete/{id}", method=RequestMethod.GET)
    public String removeProd(RedirectAttributes model,@PathVariable("id") Integer id){
        User user= (User) session.getAttribute("user");
        if(user ==null || !user.getAdmin()){
            System.out.println("nulluser");
            return "redirect:/admin/login" ;
        ...
        model.addFlashAttribute("message","Delete successful");
        return "redirect:/admin/home/prod-manage";
    }
}
