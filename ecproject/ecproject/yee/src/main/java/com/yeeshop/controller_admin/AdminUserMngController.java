...
public class AdminUserMngController {
	...
	public String delete(@RequestParam(value = "id", required = false) String id1,
			@PathVariable(value = "id", required = false) String id2, RedirectAttributes model) {
        User user= (User) session.getAttribute("user");
        if(user ==null || !user.getAdmin()){
            System.out.println("nulluser");
            return "redirect:/admin/login" ;
		...
		model.addFlashAttribute("message", "Yee Shop Notification: Remove User Successful !!!");
    ...
    public String userUpdate(RedirectAttributes model,@ModelAttribute("userentity") User user1,@RequestParam("photo_file") MultipartFile file) throws IllegalStateException, IOException{
        User user= (User) session.getAttribute("user");
        if(user ==null || !user.getAdmin()){
            System.out.println("nulluser");
            return "redirect:/admin/login" ;
        ...
        model.addFlashAttribute("message", "Yee Shop Notification: Update successful !!!");
        return "redirect:/admin/user-manage";
    }
    ...
    public String addUser(RedirectAttributes model,@ModelAttribute("entity") User user1,@RequestParam("photo_file") MultipartFile file,@RequestParam("password") String pwd ) throws IllegalStateException, IOException{
        User user= (User) session.getAttribute("user");
        if(user ==null || !user.getAdmin()){
            System.out.println("nulluser");
            return "redirect:/admin/login" ;
        } 
        if(userDAO.findById(user1.getId())!=null){
            ...
            model.addFlashAttribute("message", "Yee Shop Notification: Add new user successful");
        }
