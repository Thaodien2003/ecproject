...
public class UserController {
	...
	public String registerUser(RedirectAttributes model, @Validated @ModelAttribute("user") User user,BindingResult errorResult,// read form data request
	@RequestParam("photo_file") MultipartFile file,@RequestParam("password") String pwd,@RequestParam("cpassword") String cpwd) throws IllegalStateException, IOException, MessagingException{
		if (errorResult.hasErrors()){
			...
			if(existUser != null){
				model.addFlashAttribute("message", "This username has been used");
				return "redirect:/register";
			}
		}
		if(!pwd.equals(cpwd)){
			model.addFlashAttribute("message","Password Confirm is not correct !!! Please try again.");
			return "redirect:/register";
		}
		if (file.isEmpty()){
		...
		user.setAdmin(false);
		userDAO.create(user);
		model.addFlashAttribute("message1", "Register Successful. Please Login your new account");
		return "redirect:/login";
	}
	...
	@RequestMapping("/drawImage1")
	public ResponseEntity<byte[]> drawImage(HttpServletResponse response) {
        ...
        byte[] media = Base64.getDecoder().decode(base64);
